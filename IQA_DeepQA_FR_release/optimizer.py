...
class Optimizer(object):
    ...
    def get_updates_cost(self, cost, params, scheme='nadam', lr_factors=None):
        if scheme == 'adagrad':
            ...
            raise ValueError(
                'Select the proper scheme (%s): ' % scheme,
    ...
    def get_updates_adam(self, cost, params,
        ...
        if not lr_factors:
            lr_factors = [1.0 for i in range(len(params))]
        for p, g, factor in zip(params, grads, lr_factors):
            ...
            p_t = p - lr_t * factor * m_t / (T.sqrt(v_t) + epsilon)
    ...
    def get_updates_nadam(self, cost, params,
        ...
        for p, g, factor in zip(params, grads, lr_factors):
            ...
            p_t = p - self.lr * factor * m_t_bar / (T.sqrt(v_t_prime) + epsilon)
