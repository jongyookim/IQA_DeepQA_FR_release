...
class Model(ModelBasis):
    ...
    def init_model(self):
        ...
        self.layers[key].append(layers.ConvLayer(
            self.input_shape, 32, (3, 3), layers.lrelu, name=key + '/conv1'))

        self.layers[key].append(layers.ConvLayer(
            self.last_sh(key), 32, (3, 3), layers.lrelu, name=key + '/conv2',
            subsample=(2, 2)))

        #######################################################################
        self.layers[key].append(layers.ConvLayer(
            self.last_sh(key), 64, (3, 3), layers.lrelu, name=key + '/conv3'))
    ...
    def sens_map_fn(self, x_c):
        output = self.get_key_layers_output(x_c, 'sens_map')
        return output
    ...
    def cost_iqa(self, x, x_c, mos, n_img=None, bat2img_idx_set=None):
        ...
        sens_map = self.sens_map_fn(x_c_norm)
        ...
        records.add_imgs('x_c', x_c_im, caxis=[0, 1], scale=1.0)
        # pyr_caxis = [-0.25, 0.25]
        # records.add_imgs('x_c_mf', x_c_mf, caxis=pyr_caxis, scale=0.5)
        # records.add_imgs('x_c_hf', x_c_hf, caxis=pyr_caxis, scale=1.0)
